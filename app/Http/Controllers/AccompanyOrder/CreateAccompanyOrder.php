 ...
 */
class CreateAccompanyOrder extends Controller
{
    use ServiceCharge;
    use RangeSelect;
    ...
    protected function getPaymentObj()
    {
        if ($this->order_data['pay_sum'] == 0 || $this->order_data['pay_type'] == self::WALLET_BUY) {
            $this->PAYMENT = new WalletAccompanyOrder();
            return;
        }

        switch ($this->order_data['pay_type']) {
            case self::ALI_BUY:
                $this->PAYMENT = new AliPayAccompanyOrder();
                break;

            case self::WECHAT_BUY:
                $this->PAYMENT = new WechatPayAccompanyOrder();
                break;
    ...
    protected static function calculate($user_id, &$requestData, &$ticket = false)
    {
        ...
        $requestData['level_type'] = isset($requestData['level_type']) ? (int)$requestData['level_type'] : 1;
    ...
    protected function setData($requestData, $user_id, $calculate_data)
    {
        ...
        $game_info = Game::getGameServerLevel($game_id,$server_id,$requestData['level_id']);
        ...
        $this->order_data = [
            ...
            'server_name' => $game_info['server_name'],
            'level_name' => isset($game_info['level_name']) ? $game_info['level_name'] : '',
