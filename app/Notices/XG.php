...
class XG
{
    ...
    {
        $push = new XG($accessId, $secretKey);
        $mess = new Message();
        $mess->setTitle($title);
    ...
    public function PushSingleDevice($deviceToken, $message, $environment = 0)
    {
        ...
        if ($message instanceof MessageIOS) {
            if ($environment != XG::IOSENV_DEV && $environment != XG::IOSENV_PROD) {
...
//        $pushId = intval($pushId);
    ...
    {
        if ($this->accessId >= XG::IOS_MIN_ID and $message instanceof MessageIOS)
            return true;
        else if ($this->accessId < XG::IOS_MIN_ID and $message instanceof Message)
            return true;
        else
            return false;
    }
